...
ID <- "MIN4"
repo <- find_repo()
if (is.na(repo)) { cat("MIN4: SKIP (source tree not available)\n"); quit(save = "no", status = 0) }
HPP <- file.path(repo, "src", "Palantir_Core", "IntervalHistory.hpp")
CPP <- file.path(repo, "src", "Palantir_Core", "IntervalHistory.cpp")
