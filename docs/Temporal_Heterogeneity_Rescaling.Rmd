...
This event-budget time change applies to `substitution`,
`synonymous-per-codon`, and `non-synonymous` currencies. It is bypassed for
`dS` (including user input `scaling_type = "synonymous"`), because dS denotes
neutral evolutionary time. A selected process therefore runs for the supplied
branch duration through an equilibrium transient; forcing a realized
synonymous-event budget would change the gauge.
