...
## The exact time change (default since v1.2.2)
...
instead of segment by segment. This method (`rescale_method = "exact"`,
introduced in v1.1.0 and the default since v1.2.2; pass
`rescale_method = "segments"` for the original scheme) delivers the
...
methods. The segment scheme remains available for reproducing the original
discretized calculation.

The same time-change machinery applies to dS, but its budget is defined by a
neutral-reference clock rather than a realized event class. For each codon,
PalantiR stores one third of the neutral reference's total outflux. During a
fitness-profile or population-size transient it integrates that outflux under
the evolving codon distribution and preserves the destination mode's
stationary dS-clock rate. This keeps the supplied branch in neutral nucleotide
units without forcing one realized synonymous event per codon. The other
currencies preserve their selected event-class budgets: `substitution`,
`synonymous-per-codon`, and `non-synonymous`.
