...
For more details on time-heterogeneous simulation, see [[Tempral Heterogeneity Rescaling]].
...
psi <- equilibrium_to_fitness(aa_pi[1, ], 10000)
```
...
```{r sample_sequence}
s <- sample_sequence(ms_1k, 1)
as_amino_acid(s$sequence)
```
