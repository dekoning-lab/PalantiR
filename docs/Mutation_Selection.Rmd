...
aa_psi <- as.matrix(read.csv("../inst/extdata/amino_acid_fitness_N_1000.csv"))
```
...
Here `"synonymous"` selects the dS gauge, so the tree lengths are interpreted
as neutral nucleotide substitutions per site. Use
`"synonymous-per-codon"` only when one branch-length unit should instead mean
one expected synonymous event per codon at stationarity.
...
Synonymous substitutions are shown in green, non-synonymous in red.
