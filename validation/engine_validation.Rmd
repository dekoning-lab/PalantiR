...
# Test 1: branch-length calibration under event-count and dS scaling
...
event-count gauge. With amino-acid fitness differences removed, its neutral
total rate is three substitutions per codon, so a branch of length one should
deliver three events per codon on average.
...
per_site_ds <- tabulate(sim_ds$substitutions$site + 1L,
                        nbins = n_sites) / 2
ds_mean <- mean(per_site_ds)
ds_se <- sd(per_site_ds) / sqrt(n_sites)
ds_z <- (ds_mean - 3) / ds_se
knitr::kable(data.frame(scaling = "dS", mean = ds_mean,
                        se = ds_se, z = ds_z), digits = 4)
```

```{r test1-ds-verdict, results="asis"}
verdict("Test 1 dS", abs(ds_z) < 3,
        sprintf("neutral events per codon per unit dS: %.3f (z %.2f, criterion |z| < 3)",
                ds_mean, ds_z))
```
...
defect exceeded the early-checkpoint bounds five-fold), and Monte-Carlo
chi-square of the observed amino-acid distribution against the predicted
trajectory gives p ≥ 0.001 at every checkpoint with at most two of the
twelve p-values below 0.05.
